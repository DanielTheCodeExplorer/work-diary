...
def create_task(conn: sqlite3.Connection, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cursor = conn.execute(
        """
        INSERT INTO tasks (
            title, project, due_date, due_time, reminder_at, repeat_rule, priority,
...
def create_next_repeating_task(conn: sqlite3.Connection, task: Dict[str, Any]) -> None:
    ...
    if base_date:
        ...
        next_due_date = next_repeat_date(today_iso(), repeat_rule)
...
class WorkDiaryHandler(BaseHTTPRequestHandler):
    ...
    def redirect_authenticated_login(self, path: str) -> bool:
        if path == "/login" and self.is_authenticated_request():
            self.redirect("/")
            return True
        return False
