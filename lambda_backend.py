...
TASK_REPEAT_RULES = ["none", "daily", "weekly", "monthly"]
...
def validate_repeat_rule(value: Any) -> str:
    ...
    if repeat_rule not in TASK_REPEAT_RULES:
        raise ValidationError("Repeat must be none, daily, weekly, or monthly.")
    return repeat_rule
...
def list_tasks(filters: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
    ...
    if completed_filter in {"true", "1", "yes"}:
        return [task for task in items if task["completed"]]
    if completed_filter in {"false", "0", "no"}:
        return [task for task in items if not task["completed"]]
    return items
...
def create_next_repeating_task(task: Dict[str, Any]) -> None:
    ...
    if base_date:
        next_due_date = next_repeat_date(base_date, repeat_rule)
    else:
        next_due_date = next_repeat_date(dt.date.today().isoformat(), repeat_rule)
...
def next_repeat_date(base_date: str, repeat_rule: str) -> str:
...
def list_entries(filters: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
    filters = filters or {}
    items = [row_to_entry(item, evidence_count=0) for item in scan_table(entries_table)]
    evidence_items = [row_to_evidence(item) for item in scan_table(evidence_table)]
    evidence_counts = {}
    for evidence_item in evidence_items:
        evidence_counts[evidence_item["work_entry_id"]] = evidence_counts.get(evidence_item["work_entry_id"], 0) + 1
    for entry in items:
        entry["evidence_count"] = evidence_counts.get(entry["id"], 0)
    filtered = [entry for entry in items if entry_matches(entry, filters)]
...
def validate_url(value: Any, evidence_type: str) -> str:
    text = compact_text(value)
    if evidence_type == "uploaded_file_placeholder":
        return text
    if not text:
...
def list_evidence(filters: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
    filters = filters or {}
    entries = {item["id"]: row_to_entry(item) for item in scan_table(entries_table)}
    items: List[Dict[str, Any]] = []
    for item in scan_table(evidence_table):
        evidence_item = row_to_evidence(item)
...
def get_options() -> Dict[str, Any]:
    entries = [row_to_entry(item) for item in scan_table(entries_table)]
